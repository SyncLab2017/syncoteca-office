...
_LABEL_CONTEXT_RE = re.compile(
    ...
    r"правообладател[яьей]+|рекорд-лейбл[аеыуой]?)\s+([«»\w\s.,-]+?)"
    r"(?=\s+(?:год|за|и|дай|скинь|в\s+(?:нашей|базе|каталоге|реестре)|"
    r"содержит|треков|есть|нет|у\s+нас|сколько|на\s+данный|имеет|имеется)|[.,!?]|$)",
    re.IGNORECASE,
)
...
def parse_export_query(text: str) -> dict:
    ...
    if not filters.get("artist") and not filters.get("label") and not filters.get("genre") and not (_year_found and _text_long):
        stop = {
            ...
            "релиз", "релизе", "релиза", "релизы", "релизов", "release",
            ...
            "иностранные", "иностранная", "иностранный", "иностранных", "иностранным", "иностранного", "иностранную",
